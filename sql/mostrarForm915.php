...
if (isset($cons['cantUF'])){
  $uf = $cons['cantUF'];
  }else {
  $uf = "";
}

$strdate =  strtotime($fechaImp);

$idobra = $cons['codObra'];

$Prof = mysqli_fetch_array(mysqli_query($conexion,"SELECT * FROM obras o, profesionales p WHERE (o.codObra='$idobra') AND (o.idProfesional=p.idProfesional)"));

    $PnombreApellido= $Prof['pnombreApellido'];
    $Pmatricula = $Prof['matricula'];
//DATOS FORMULARIO 915
$row915 = mysqli_fetch_array(mysqli_query($conexion,"SELECT * FROM cedulaformularios o, form915 f WHERE o.idCedula= $idcedula AND o.tipoCedula = $cedula AND o.codForm = $idform AND o.codForm = f.codForm915"));
