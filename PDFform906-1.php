...
include("sesion.php");
	...
	$Aire=mysqli_fetch_array(mysqli_query($conexion,"SELECT * FROM `rub6data` WHERE `codForm`= $post AND `prop`= 'Aire' AND `numForm`= '906'"));
	if ($Aire['cant']==0) {
		foreach ($Aire as $key => $value) {
			$Aire[$key] = NULL;
		}
	}
	$Calef=mysqli_fetch_array(mysqli_query($conexion,"SELECT * FROM `rub6data` WHERE `codForm`= $post AND `prop`= 'Calef' AND `numForm`= '906'"));
	if ($Calef['cant']==0) {
		foreach ($Calef as $key => $value) {
			$Calef[$key] = NULL;
		}
	}
	$Losa=mysqli_fetch_array(mysqli_query($conexion,"SELECT * FROM `rub6data` WHERE `codForm`= $post AND `prop`= 'Losa' AND `numForm`= '906'"));
	if ($Losa['cant']==0) {
		foreach ($Losa as $key => $value) {
			$Losa[$key] = NULL;
		}
	}
	$Incen=mysqli_fetch_array(mysqli_query($conexion,"SELECT * FROM `rub6data` WHERE `codForm`= $post AND `prop`= 'Incen' AND `numForm`= '906'"));
	if ($Incen['cant']==0) {
...
<html lang="es">
	...
	<body>
		<div class="imagen">
			<img src="img/logopdf906hoja1.png">
		</div>
		<div class="version">
				/<?php echo $version ?>
			</div>
		...
		<div class="rub5">
			...
			<table class="table table-rub5">
				...
				<tr>
					...
					<td align="center"><?php Mostrar (number_for($TotalCub,$Cubierta),2)  ?></td>
				</tr>
			</table>
		</div>
		<div class="rub6">
